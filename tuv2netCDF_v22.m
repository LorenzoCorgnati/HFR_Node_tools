...
function [T2C_err,networkData,ncFileNoPath,ncFilesize] = tuv2netCDF_v22(totFilename,timestamp,networkData,networkFields,stationData,stationFields)

disp(['[' datestr(now) '] - - ' 'tuv2netCDF_v22.m started.']);
...
try
    % Load total file as table
    totTable = load(totFilename);
    
    % Load the total file as structure
    totStruct = loadRDLFile(totFilename,'false','warning');
catch err
...
%% Retrieve the column names of the data table and site codes and coordinates
...
try
    ...
    [T2C_err, TUV] = totTable2TUV(totTable,tableFields,timestamp,siteLon,siteLat);
catch err
    disp(['[' datestr(now) '] - - ERROR in ' mfilename ' -> ' err.message]);
    T2C_err = 1;
end
...
try
    ...
    stationIDs = stationData(:,station_idIndex);
    
%     % Find the index of the operational_to field
...
%     operational_to = stationData(:,operational_toIndex);
...
%     stationIDs = stationIDs(ST_last_calibration_date~='null');
    lastPatternStr = [stationIDs{1,:} ': '];
    for lcd_idx=2:length(ST_last_calibration_date)
        lastPatternStr = [lastPatternStr datestr(ST_last_calibration_date(lcd_idx-1), 'yyyy-mm-dd') 'T' datestr(ST_last_calibration_date(lcd_idx-1), 'HH:MM:SS') 'Z; ' stationIDs{lcd_idx,:} ': '];
    end
    ...
    for doa_idx=2:length(ST_DoA)
        DoAStr = [DoAStr ST_DoA{doa_idx} '; ' stationIDs{doa_idx,:} ': '];
    end
    ...
    calibration_typeStr = [stationIDs{1,:} ': '];
    for ct_idx=2:length(ST_calibration_type)
        calibration_typeStr = [calibration_typeStr ST_calibration_type{ct_idx-1} '; ' stationIDs{ct_idx,:} ': '];
    end
    ...
    calibration_linkStr = [stationIDs{1,:} ': '];
    for cl_idx=2:length(ST_calibration_link)
        calibration_linkStr = [calibration_linkStr ST_calibration_link{cl_idx-1} '; ' stationIDs{cl_idx,:} ': '];
    end
...
try
    ...
    coverageStart = addtodate(TUVgrid.TimeStamp, -temporal_resolution/2, 'minute');
    timeCoverageStart = [datestr(coverageStart, 'yyyy-mm-dd') 'T' datestr(coverageStart, 'HH:MM:SS') 'Z'];
    coverageEnd = addtodate(TUVgrid.TimeStamp, temporal_resolution/2, 'minute');
...
try
    ...
    dataID = [EDIOS_Series_ID '-Total_' datestr(TUVgrid.TimeStamp, 'yyyy-mm-dd') 'T' datestr(TUVgrid.TimeStamp, 'HH:MM:SS') 'Z'];
...
try
    ...
    [overall_QCflag, varianceThreshold_QCflag, temporalDerivativeThreshold_QCflag, GDOP_QCflag, dataDensity_QCflag, velocityThreshold_QCflag] = tuvTotalQCtests_v11(TUVgrid, Total_QC_params);
catch err
    disp(['[' datestr(now) '] - - ERROR in ' mfilename ' -> ' err.message]);
    T2C_err = 1;
end
...
try
    ...
    sdnPosition_QCflag = netcdf.getConstant('NC_FILL_BYTE').*int8(ones(size(TUVgrid.gridLat)));
    sdnPosition_QCflag(~isnan(TUVgrid.U_grid)) = 1;
...
try
    ...
    ncwriteatt(ncfile,'CCOV','data_mode',char('R'));
    ...
    ncwrite(ncfile,'TIME',TUVgrid.TimeStamp-timeref);
...
if(T2C_err==0)
    disp(['[' datestr(now) '] - - ' 'tuv2netCDF_v22.m successfully executed.']);
end
