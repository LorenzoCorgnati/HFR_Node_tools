...
function [cA2C_err,networkData,ncFileNoPath,ncFilesize] = curAsc2netCDF_v33(totFilename,timestamp,networkData,networkFields,stationData,stationFields)

disp(['[' datestr(now) '] - - ' 'curAsc2netCDF_v33.m started.']);
...
if(cA2C_err == 0)
    ...
    try
        ...
        ncwriteatt(ncfile,'VART_QC','comment',char(['OceanSITES quality flagging for variance threshold QC test. ' ...
            'Threshold set to ' num2str(Total_QC_params.VarThr) ' m/s. ']));
...
if(cA2C_err==0)
    disp(['[' datestr(now) '] - - ' 'curAsc2netCDF_v33.m successfully executed.']);
end
