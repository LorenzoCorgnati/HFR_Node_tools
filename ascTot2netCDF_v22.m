...
function [cA2C_err,networkData,ncFileNoPath,ncFilesize] = ascTot2netCDF_v22(totFilename,timestamp,networkData,networkFields,stationData,stationFields)

disp(['[' datestr(now) '] - - ' 'ascTot2netCDF_v22.m started.']);
...
try
    ...
    ncwriteatt(ncfile,'UACC','coordinates',char('TIME DEPTH LATITUDE LONGITUDE'));
    ...
    ncwriteatt(ncfile,'VACC','coordinates',char('TIME DEPTH LATITUDE LONGITUDE'));
...
if(cA2C_err==0)
    disp(['[' datestr(now) '] - - ' 'ascTot2netCDF_v22.m successfully executed.']);
end
