%% ruv2netCDF_v33.m
% This function converts hourly radial files from native Codar format .ruv
...
function [R2C_err,networkData,stationData,ncFileNoPath,ncFilesize,station_tbUpdateFlag] = ruv2netCDF_v33(HFRP_RUV,networkData,networkFields,stationData,stationFields,timestamp)

disp(['[' datestr(now) '] - - ' 'ruv2netCDF_v33.m started.']);
...
if(R2C_err==0)
    disp(['[' datestr(now) '] - - ' 'ruv2netCDF_v33.m successfully executed.']);
end
