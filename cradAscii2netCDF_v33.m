...
function [cA2C_err,networkData,ncFileNoPath,ncFilesize] = cradAscii2netCDF_v33(totFilename,timestamp,networkData,networkFields,stationData,stationFields)

% CONTROLLARE TUTTO
disp ('TUTTO DA RIVEDERE PER SOSTITUIRE DOVE SERVE networkData con stationData');

disp(['[' datestr(now) '] - - ' 'cradAscii2netCDF_v33.m started.']);
...
try
    ...
    % Publication information    ncwriteatt(ncfile,'/','Conventions',char('CF-1.6, OceanSITES-Manual-1.2, Copernicus-InSituTAC-SRD-1.4, CopernicusInSituTAC-ParametersList-3.1.0, Unidata, ACDD, INSPIRE'));
...
if(cA2C_err==0)
    disp(['[' datestr(now) '] - - ' 'cradAscii2netCDF_v33.m successfully executed.']);
end
